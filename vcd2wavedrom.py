#####################################################################################
### Source file copied from the repository: https://github.com/Toroid-io/vcd2wavedrom
### commit: 7caf6d5e0b9f85bff943ddea928818e62c5d9e9e
###
### I copied this here because their current implementation had an unusable interface
### (install then run as script?!) and I needed to import it into my server source 
### as a library. Keeping the implementation as close to the original as possible.
###
...
def run_vcd2wavedrom(vcd_filepath, filter_signal_list):
    config = {}
    config['input'] = vcd_filepath
    config['filter'] = filter_signal_list
    config['skin'] = "narrow"
    try:
        with open(vcd_filepath, 'r') as f:
            config['input_text'] = f.read()
    except FileNotFoundError:
        print(f'ERROR: File {vcd_filepath} not found!')
        exit(1)

    vcd = VCD2Wavedrom(config)
    drom = vcd.execute(True) # True = automatic wavedrom config
    drom['config'] = {'hscale': 1, 'skin':'narrow'}
    return json.dumps(drom, indent=4)
