...
def home_get(request: Request):
    # ToDo: checksubmissions functionality (submit username-password to get table of HW vs. Qs with status notes)
    return templates.TemplateResponse('checksubmissions.html', context={'request': request})
...
def hw1_q1_post(request: Request, password: str = Form(""), username: str = Form(""), input_text: str = Form(""), buttonaction: str = Form("")):
    output = handle_hw_question(timestr  = time.strftime("%Y%m%d_%H%M%S"), 
